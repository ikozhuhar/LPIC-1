...
### /sys
Sysfs - экспортирует информацию о драйверах, устройствах и данных ядра на уровень пользователя, хранение информации об устройстве связанном с оборудованием. Монтируется в /sys. Напирмер, `devices/` - _хранит все устройства зарегистрированные в системе_, `bus/` - _перечень шин_, `drivers` - _драйверы_, `block` - _каталог блочных устройств_, `class` - _гурупировка устройств по классам_.
